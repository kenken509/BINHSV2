    ...
    <DashboardLayout :user="user">
        <div class="flex flex-col ">
            <div class="flex justify-between items-center justify-between border-bot-only border-gray-600  py-2">
                <div class="">
                    <span class="text-[20px] font-bold text-gray-500">Edit Page</span>
                </div>
            ...
            <form @submit.prevent="submit" id="submitForm">
                ...
                <div class="mb-4">
                    ...
                    <div v-if="existingImage" class="flex justify-center items-center border border-gray-300 rounded-md p-2 shadow-md mt-2" >
                        <img :src="'/storage/'+existingImage" alt="Error" class="w-[50%] h-[50%] rounded-md relative"/>
                    </div>                    
...
const form = useForm({
    id:data.fileToEdit.id,
    title:data.fileToEdit.title,
    description:data.fileToEdit.description,
    image:data.fileToEdit.image,
    threeDLink:data.fileToEdit.threeDLink,
})

const existingImage = ref(data.fileToEdit.image)
const imageUrl = ref(null);
const threeDImageFileName = ref(null);
const imageValidator = ref(null);
const imageIsValid = ref(true);
            ...
            async function validate(){
                    ...
                    if(googleDriveRegex.test(form.threeDLink))
                    {
                        ...
                        else
                        {
                            ...
                            threeDValidator.value = "";
                            isLoading.value = false;
                            form.post(route('3d.pending3dUpdate', {preserveScroll: true}))
                        }
